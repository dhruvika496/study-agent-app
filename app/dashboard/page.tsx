...
export default async function DashboardPage() {
  ...
  const uniqueSubjects = new Set(concepts.map((item) => item.subject).filter(Boolean)).size
  const averageScore = total ? concepts.reduce((sum, item) => sum + getProgressValue(item.mastery_level), 0) / total : 0
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100">
      <nav className="border-b border-slate-800 bg-slate-950/95 px-4 py-4 sm:px-6 lg:px-8">
        <div className="mx-auto flex max-w-6xl items-center justify-between">
          ...
          <div className="flex gap-3 text-sm">
            <a href="/" className="rounded-full border border-slate-700 bg-slate-900 px-4 py-2 text-slate-100 transition hover:border-slate-500 hover:bg-slate-800">Chat</a>
            <a href="/dashboard" className="rounded-full border border-slate-700 bg-slate-900 px-4 py-2 text-slate-100 transition hover:border-slate-500 hover:bg-slate-800">Dashboard</a>
          </div>
        </div>
      </nav>
      ...
      <main className="px-4 py-8 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-6xl space-y-8">
          <section className="rounded-3xl border border-slate-800 bg-slate-900/90 p-6 shadow-2xl shadow-slate-950/40">
